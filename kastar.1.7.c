...
// #define PIvalue 3.141592653 // valor de pi [Camila] use o M_PI do pacote matemático
#define TRAD ( M_PI / 180.0) // conversão de graus para radianos
#define NMAX 3              // numero de corpos - sempre +1 do numero de corpos totais. [Camila] tirei o 3º corpo para teste
...
/*  for (int i = 1; i <NMAX; ++i)
  ...
  for (t = 0; t <= t_sim; t = t + dt){
    ...
    for (i = 0; i <= NMAX; i++){
      ...
      for( k = 1 ; k <= NDIM ; k++) printf("%g ", r[i][k]/ aluz  ); // $4 $5 $6 plot depois set view equal (em anos-luz)
    ...
    for (i = 0; i <= NMAX; i++) // NÃO ESTA FALTANDO "{}" ?, PQ NAO ESTA?
      for (k = 1; k <= NDIM; k++) // [Camila] - não precisa de {} quando só há uma linha no for,  ou para for's encadeados como aqui
        a[i][k] = 0.0;            
    //----------------------------------------------------------------------------------
