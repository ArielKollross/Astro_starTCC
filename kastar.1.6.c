...
#define PIvalue 3.141592653 // valor de pi 
#define TRAD ( PIvalue / 180.0) // conversão de graus para radianos
#define NMAX 3              // numero de corpos - sempre +1 do numero de corpos totais.
#define NDIM 3              // Dimensões espaciais
#define MSUM 1.9891e30      //[kg] massa solar

//====================================================================================

// ultima att: 23/10/2019 - 22:56
...
int main(int argc, char *argv[]){
  ...
  double t_sim;                 // [  ]
  double dt;                    // [  ]
  double mass[NMAX];        // [  ]massa da estrela
  double r[NMAX][NDIM + 1]; // [  ]posição no espaço cartesiano (x,y,z)
  double v[NMAX][NDIM + 1]; // [  ]velocidade em três componentes (Vx,Vy,Vz)
  double a[NMAX][NDIM + 1]; // [  ]aceleração em 3D(x,y,z)
  //-----------------------------------------------------------------------------------
  ...
  double Rs[NMAX]; // [  ]modulo da distância estrela-sol
  double x;
  //--------------------------------------------------------------------------------
  // Novos parametros para gravitação

  double r_a[NMAX][NDIM + 1]; // [  ]vetor posição anterior
  ...
  for (i = 1; i <= NMAX; i++){
    ...
    mass[i] = newscanf_le_numero(0,"massa" ) * MSUM ;
...
/*  for (int i = 1; i <NMAX; ++i)
  ...
  printf("ident: %d \n", i);
  printf("Massa : %g\n", mass[i] / MSUM );
  Rs[i] = (sqrt(r[i][1] * r[i][1] + r[i][2] * r[i][2] + r[i][3] * r[i][3])/aluz );
  printf("Vetor distância anos-luz; %g\n", Rs[i]);
  }*/
  ...
  for (t = 0; t <= t_sim; t = t + dt){
    ...
    for (i = 0; i <= NMAX; i++){
      // impressão de dados
      printf("%g ", t / ANO); //[ano] $1
      printf("%d ", i);// $2 identificador
      printf("%g ", (sqrt(r[i][1] * r[i][1] + r[i][2] * r[i][2] + r[i][3] * r[i][3]))/ aluz ); // $3
      for( k = 1 ; k <= NDIM ; k++) printf("%g ", r[i][k]/ aluz  ); // $4 $5 $6 plot depois set view equal
      for( k = 1 ; k <= NDIM ; k++) printf("%g ", v[i][k]/1e3       ); //[km/s] $7 $8 $9 
      printf("%g ", mass[i]/MSUM );
      printf("\n ");
    } // end for i
    ...
    for (i = 0; i <= NMAX; i++) // NÃO ESTA FALTANDO "{}" ?, PQ NAO ESTA?
      for (k = 1; k <= NDIM; k++)
        a[i][k] = 0.0;
    //----------------------------------------------------------------------------------
    ...
    for (i = 0; i < NMAX; i++){
      for (j=i+1; j <= NMAX; j++){
        ...
        for (k = 1; k <= NDIM; k++){
          a[i][k] -= aux * mass[i] * dr[k]; // k-aceleração gravitacional
          a[j][k] += aux * mass[j] * dr[k]; // k-aceleração gravitacional
        }// end k
      }// end j
